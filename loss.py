...
class JDAMLoss:
    ...
    def __init__(self, sde, eps=1e-2, train=True):
        ...
        self.num_t = 100 # number of time steps to sample from
        self.t_vals = tf.linspace(0.0, 1.0, self.num_t) # time values for training
    ...
    def compute_loss(self, score_func, pet_clean, mri_clean, t=None):
        ...
        if t is None:
            if self.num_t == 1:
                t_idx = tf.zeros([batch_size], dtype=tf.int16)
                t = tf.gather(self.t_vals, t_idx)
                # tf.print("Current timestep: ", t)
            else:
                t_idx = tf.random.uniform([batch_size], minval=0, maxval=self.num_t-1, dtype=tf.int32) # random time step t uniformly in [eps, T]
                t = tf.gather(self.t_vals, t_idx)
